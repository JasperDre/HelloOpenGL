...
struct Vertex
{
	...
	{
		return myPosition == aOther.myPosition && myNormal == aOther.myNormal && myTangent == aOther.myTangent && myTextureCoordinates == aOther.myTextureCoordinates;
	}
};
...
namespace std
{
	template<> struct hash<Vertex>
	{
		...
		{
			size_t test = 0;
			CombineHash(test, aVertex.myPosition, aVertex.myNormal, aVertex.myTextureCoordinates);
			return test;
		}
	};
}
