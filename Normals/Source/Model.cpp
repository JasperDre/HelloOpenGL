...
void Model::LoadFBX(const std::string aPath)
{
    ...
    unsigned int flags = aiProcess_Triangulate | aiProcess_FlipUVs | aiProcess_JoinIdenticalVertices | aiProcess_CalcTangentSpace;
...
{
    for (int vertexIndex = 0; vertexIndex < myMeshes[0].myIndices.size(); vertexIndex += 3)
    {
        Vertex& vertex0 = myMeshes[0].myVertices[vertexIndex + 0];
        Vertex& vertex1 = myMeshes[0].myVertices[vertexIndex + 1];
        Vertex& vertex2 = myMeshes[0].myVertices[vertexIndex + 2];

        glm::vec3 edge1 = vertex1.myPosition - vertex0.myPosition;
        glm::vec3 edge2 = vertex2.myPosition - vertex0.myPosition;

        float deltaU1 = vertex1.myTextureCoordinates.x - vertex0.myTextureCoordinates.x;
        float deltaV1 = vertex1.myTextureCoordinates.y - vertex0.myTextureCoordinates.y;
        float deltaU2 = vertex2.myTextureCoordinates.x - vertex0.myTextureCoordinates.x;
        float deltaV2 = vertex2.myTextureCoordinates.y - vertex0.myTextureCoordinates.y;
        ...
        vertex0.myTangent += tangent;
        vertex1.myTangent += tangent;
        vertex2.myTangent += tangent;
    }

    for (Vertex& vertex : myMeshes[0].myVertices)
    {
        vertex.myTangent = glm::normalize(vertex.myTangent);
    }
}
